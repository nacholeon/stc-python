...
try:
    ...
    xml_file = Path(r'C:/stc_tests/IPv4_ISIS_4Ports_QoS_dot1Q_Seq_Gen.xml')

    exported_test_xml_file = Path(r'C:/stc_tests/Export_IPv4_ISIS_4Ports_QoS_Seq_Gen.xml')
    ...
    # to use sequencer set variable below to 1, otherwise use 0

    useSeq = "1"
    result_filename = "c:test.db"
    ...
    stc.perform("loadfromxml", filename="IPv4_ISIS_4Ports_QoS_dot1Q_Seq_Gen.xml")
    ...
    # Get_GC2 = stc.get("generatorconfig2")
    # print("\n Generator Config 2")
    # pprint.pprint(Get_GC2,width=1)
    ...
    print("\nConfigure the correct absolute resultsdirectory")
    ...
    if useSeq == "1":
        ...
        Get_Seq = stc.get(seq)
        print("\nSequencer1 Params")
        pprint.pprint(Get_Seq, width=1)
        Get_Seq_State = stc.get("system1.sequencer", "state")
        print("\n\n\nSequencer State:", Get_Seq_State)

        seqchild = stc.get("system1.sequencer", "children")
        seqchildlist = seqchild.split()
        print("\n Sequencer Children: ")
        pprint.pprint(seqchildlist, width=1)
        ...
        while (stc.get(seq, "TestState") == "NONE"):
            ...
            time.sleep(2)
            print("\nSequencer State: ", Get_Seq_State)
            print("\nSequencer is executing command ", stc.get(seq, "CurrentCommand"))
            print("\nTestState: ", stc.get(seq, "TestState"))
        ...
        stc.perform('resultssubscribe', Parent="project1", ConfigType="Analyzer", resulttype="AnalyzerPortResults",
                    filenameprefix="AnalyzerPortResults")
        stc.perform('resultssubscribe', Parent="project1", ConfigType="Generator", resulttype="GeneratorPortResults",
                    filenameprefix="GeneratorPortResults")

        print("subscribe to results at streamblock level")

        stc.perform('resultssubscribe', Parent="project1", ConfigType="StreamBlock", ResultType="TxStreamBlockResults",
                    FilenamePrefix="TxStreamBlockResults", Interval="2")
        stc.perform('resultssubscribe', Parent="project1", ConfigType="StreamBlock", ResultType="TxStreamResults",
                    FilenamePrefix="TxStreamResults", Interval="2")
        stc.perform('resultssubscribe', Parent="project1", ConfigType="StreamBlock", ResultType="RxStreamBlockResults",
                    FilenamePrefix="RxStreamBlockResults", Interval="2")
        stc.perform('resultssubscribe', Parent="project1", ConfigType="StreamBlock",
                    ResultType="RxStreamSummaryResults", FilenamePrefix="RxStreamSummaryResults", Interval="2")

        # subscribe ISIS results
        print("subscribe to results at ISIS level")
        stc.perform('resultssubscribe', Parent="project1", resulttype="IsisRouterResults",
                    ConfigType="IsisRouterConfig")
        ...
        stc.perform("WaitForRouterStateCommand", ObjectList=xml_isis_router_config_str, WaitRouterState='PROTOCOL_UP',
                    WaitTime="60")
        ...
        stc.apply()
        #        # show some counters from the devices
        #       for ii in xmldhcpv4blockconfiglist:
        #            print("There are ", stc.get(stc.get(ii,"resultchild-Targets"),"CurrentBoundCount") ," session bound.")
        #        for ii in xmldhcpsrvconfiglist:
        #            srvresh = stc.get(ii,'children-dhcpv4serverresults')
        #            print("The DHCP server", stc.get(stc.get(srvresh,'parent'),'Name'), " assigned ", stc.get(srvresh,'CurrentBoundCount') ," addresses.")
        # Print for each DHCP session the mac@ and corresponding ip@
        # Save all dhcp client session information in a csv file
        #        for ii in xmldhcpv4blockconfiglist:
        #            stc.perform("dhcpv4sessionInfo",Blocklist=ii)
        #            lstDhcpv4SessionResults = stc.get(ii,"children-dhcpv4sessionresults")
        #            for hDhcpv4SessionResult in lstDhcpv4SessionResults.split(" "):
        #            sesState = stc.get(hDhcpv4SessionResult,"SessionState")
        #            sesIp = stc.get(hDhcpv4SessionResult,"LastBoundIpv4Addr")
        #            sesMac = stc.get(hDhcpv4SessionResult,"MacAddr")
        #            print("The DHCP session with mac address ", sesMac ," is in the state ", sesState ," and has the ip address ", sesIp ," assigned.")
            ...
            for ii in txresultlist:
                for jj in rxresultlist:
                    if stc.get(ii, "StreamId") == stc.get(jj, "Comp32"):
                        print("stream", stc.get(stc.get(ii, "parent"), 'Name'), "(streamid ", stc.get(ii, "StreamId"),
                              " send ", stc.get(ii, "FrameRate"), " fps and received ", stc.get(jj, "FrameRate"),
                              " fps with short term average latency ", stc.get(jj, "ShortTermAvgLatency"), " us")
                        print("stream", stc.get(stc.get(ii, "parent"), 'Name'), "(streamid ", stc.get(ii, "StreamId"),
                              " send ", stc.get(ii, "framecount"), " packets and received ", stc.get(jj, "framecount"),
                              " packets with average latency ", stc.get(jj, "AvgLatency"), " us")
            i -= 1
            print("\r")
        time.sleep(5)
        for ii in xml_port_list:
        ...
        for ii in txblockresultlist:
            for jj in rxblockresultlist:
                if stc.get(ii, "parent") == stc.get(jj, "parent"):
                    print("streamblock ", stc.get(stc.get(ii, "parent"), 'Name'), " send ", stc.get(ii, "framecount"),
                          " packets and received ", stc.get(jj, "framecount"), " packets with average latency ",
                          stc.get(jj, "AvgLatency"), " us")
